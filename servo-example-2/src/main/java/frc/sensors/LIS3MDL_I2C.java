...
public class LIS3MDL_I2C implements Angle, Sendable {
    ...
    // Output Data Rate
    // these are bits 2-4 in CTRL_REG1
    ...
    // Sensitivity
    // these are bits 5-6 in CTRL_REG2
    ...
    // Operating Mode for X and Y
    // these are bits 5-6 in CTRL_REG1
    ...
    public LIS3MDL_I2C() {
        m_i2c = new I2C(I2C.Port.kMXP, LIS3MDL_MAG_I2C_ADDRESS_LOW >>> 1);
        setODR(LIS3MDL_MAG_DO_T.LIS3MDL_MAG_DO_80Hz); // fastest
        setFS(LIS3MDL_MAG_FS_T.LIS3MDL_MAG_FS_4Ga); // most sensitive
        setOperatingModeXY(LIS3MDL_MAG_OM_T.LIS3MDL_MAG_OM_HIGH);
        enable(LIS3MDL_MAG_MD_T.LIS3MDL_MAG_MD_CONTINUOUS);
    }

    public void setODR(LIS3MDL_MAG_DO_T newValue) {
    ...
    public void setFS(LIS3MDL_MAG_FS_T newValue) {
        ByteBuffer buf = ByteBuffer.allocate(1);
        buf.order(ByteOrder.LITTLE_ENDIAN);
    ...
    public void setOperatingModeXY(LIS3MDL_MAG_OM_T newValue) {
        ByteBuffer buf = ByteBuffer.allocate(1);
        buf.order(ByteOrder.LITTLE_ENDIAN);
        m_i2c.read(LIS3MDL_MAG_CTRL_REG1, 1, buf);
        byte value = buf.get();
    ...
    public void enable(LIS3MDL_MAG_MD_T newValue) {
        ByteBuffer buf = ByteBuffer.allocate(1);
        buf.order(ByteOrder.LITTLE_ENDIAN);
